...
- Tests do not mirror `src/` files. `test/runtests.jl` always runs `test/icgem.jl` and `test/gravity_models.jl`; on non-prerelease Julia it additionally `Pkg.add`s DifferentiationInterface, ForwardDiff, Zygote, JET, AllocCheck, and Aqua at runtime and runs `test/zygote_ext.jl` plus `test/allocations.jl` (the JET check and, on macOS, the allocation tests are skipped only on Julia 1.12, where those tools do not work).
